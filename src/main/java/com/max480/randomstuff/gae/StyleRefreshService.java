...
/**
 * A (pretty silly) service aiming to change my GB profile background on a daily basis (called every day at midnight by a crontab).
 * The result is sent to https://storage.googleapis.com/max480-random-stuff.appspot.com/gamebanana-profile-background.css
 */
@WebServlet(name = "StyleRefreshService", urlPatterns = {"/gamebanana-css-refresh"})
public class StyleRefreshService extends HttpServlet {
    private final Logger logger = Logger.getLogger("StyleRefreshService");
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        if (("key=" + SecretConstants.CATALOG_RELOAD_SHARED_SECRET).equals(request.getQueryString())) {
            ...
            String css = "/* Generated on " + ZonedDateTime.now(ZoneId.of("UTC")).format(DateTimeFormatter.ofPattern("MMMM d, yyyy HH:mm zzz", Locale.ENGLISH)) + "" +
                    " by https://github.com/max4805/RandomStuffWebsite/blob/main/src/main/java/com/max480/randomstuff/gae/StyleRefreshService.java */\n";
            ...
            BlobId blobId = BlobId.of("max480-random-stuff.appspot.com", "gamebanana-profile-background.css");
            BlobInfo blobInfo = BlobInfo.newBuilder(blobId).setContentType("text/css").build();
            storage.create(blobInfo, css.getBytes(StandardCharsets.UTF_8));
