...
<p>
    This page can give you direct download URLs to the latest version of a mod, based on the <code>Name</code> present
    in its <code>everest.yaml</code> file.
</p>
<p>
...
<div class="alert alert-info">
    ...
    <ul>
        <li>
            ...
            downloads the latest stable version of Olympus for Windows. Other options for branches are <code>main</code>
            and <code>windows-init</code>,
...
<form method="POST">
    <div class="form-group">
        <label for="modId">Type a mod's <code>everest.yaml</code> name here (<b>make sure that uppercase/lowercase
            letters are correct</b>):</label>
        <input type="text" class="form-control" id="modId" name="modId" required
               value="<%= escapeHtml4((String) request.getAttribute("typedId")) %>">
    </div>

    <div class="form-check">
        <input type="checkbox" class="form-check-input" id="bundle" name="bundle"
               value="" <% if ((boolean) request.getAttribute("bundle")) { %> checked <% } %>>
        <label class="form-check-label" for="bundle">Get a zip with the mod and all of its dependencies</label>
    </div>

    <div class="form-check">
        <input type="checkbox" class="form-check-input" id="twoclick" name="twoclick"
               value="" <% if ((boolean) request.getAttribute("twoclick")) { %> checked <% } %>>
        <label class="form-check-label" for="twoclick">Link to 0x0ade's 2-click installer</label>
    </div>

    <div class="form-check">
        <input type="checkbox" class="form-check-input" id="mirror" name="mirror"
               value="" <% if ((boolean) request.getAttribute("mirror")) { %> checked <% } %>>
    ...
    <div class="alert alert-danger">
        <b>The <code>everest.yaml</code> name you specified
            (<code><%= escapeHtml4((String) request.getAttribute("typedId")) %>
            </code>) was not found!</b>
        Please try again.
    </div>
    <% } %>
...
<div class="alert alert-secondary">
    If you have an <code>everest.yaml</code> name for a mod and want to link to its <i>GameBanana page</i> (<b>not</b> a
    direct file download), use this address:
