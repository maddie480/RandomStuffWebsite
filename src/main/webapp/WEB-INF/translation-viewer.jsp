...
<h1 class="mt-4"><%= request.getAttribute("title") %>
</h1>
...
<p>
    ...
    <a href="https://github.com/EverestAPI/Everest/tree/dev/Celeste.Mod.mm/Content/Dialog" target="_blank">the Everest
        dialog files</a>
...
<div class="toggle">
    <a href="?program=everest" role="button"
       class="btn <%= "everest".equals(request.getParameter("program")) ? "btn-primary" : "btn-link" %>">Everest</a>
    <a href="?program=olympus" role="button"
       class="btn <%= "olympus".equals(request.getParameter("program")) ? "btn-primary" : "btn-link" %>">Olympus</a>
    <a href="?program=cu2" role="button"
       class="btn <%= "cu2".equals(request.getParameter("program")) ? "btn-primary" : "btn-link" %>">Collab Utils 2</a>
    <a href="?program=evm" role="button"
       class="btn <%= "evm".equals(request.getParameter("program")) ? "btn-primary" : "btn-link" %>">Extended Variant
        Mode</a>
</div>
...
<table class="table">
    <tr>
        <th>Dialog ID</th>
        <th><%= request.getAttribute("leftLang") %>
        </th>
        <th>
    ...
    <tr>
        <td><code><%= escapeHtml4(entry.getLeft()) %>
        </code></td>
        <td>
            <% if (entry.getMiddle().isEmpty()) { %>
            <b>Missing</b>
            <% } else { %>
            <code><%= escapeHtml4(entry.getMiddle()) %>
            </code>
            <% } %>
        </td>
        <td>
            ...
            <b>Missing</b>
            <% } else { %>
            <code><%= escapeHtml4(entry.getRight()) %>
            </code>
            <% } %>
        </td>
    </tr>
