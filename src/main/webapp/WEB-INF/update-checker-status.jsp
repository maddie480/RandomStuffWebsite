<%@ page
        import="java.util.List, java.text.DecimalFormat, static org.apache.commons.text.StringEscapeUtils.escapeHtml4, static ovh.maddie480.randomstuff.frontend.UpdateCheckerStatusService.LatestUpdatesEntry" %>
...
<p>
    The database does <i>fast updates</i> <b>every 2 minutes</b>. It only goes through mods that were modified since the
    last update.
    <br>
    ...
    <%= new DecimalFormat("0.0").format((double) request.getAttribute("lastIncrementalCheckDuration") / 60.0) %>
    minutes.
    <% } else { %>
...
<p>
    ...
    </b> mods are currently registered.
</p>

<div class="alert alert-info">
    <p>
        You can learn more about the files generated by the Everest Update Checker by checking its README
        <a href="https://github.com/maddie480/EverestUpdateCheckerServer/blob/master/README.md" target="_blank">on
            GitHub</a>.
    </p>
    <p>
        APIs that provide mod search and sorted / filtered lists are also available,
        <a href="https://github.com/maddie480/RandomStuffWebsite/blob/main/README.md#gamebanana-search-api"
           target="_blank">check the docs here</a>.
    </p>
    <p>
...
<table class="table table-striped">
    ...
    <tr>
        ...
        <td>
            <% if (entry.isAddition) { %>
            <a href="<%= escapeHtml4(entry.url) %>" target="_blank"><b><%= escapeHtml4(entry.name) %>
            </b></a>
            was updated to version <b><%= escapeHtml4(entry.version) %>
        </b>
            <% } else { %>
            <b><%= escapeHtml4(entry.name) %>
            </b> was deleted
            <% } %>
        </td>
    </tr>
