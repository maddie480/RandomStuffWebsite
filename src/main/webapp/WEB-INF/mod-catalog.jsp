<%@ page
        import="java.util.List, java.util.Map, ovh.maddie480.randomstuff.frontend.CelesteModCatalogService, static org.apache.commons.text.StringEscapeUtils.escapeHtml4" %>

<% if ((boolean) request.getAttribute("error")) { %>
<div class="alert alert-danger">
    An error occurred while loading the entity and trigger list. Please try again later.<br>
    If this keeps happening, get in touch with maddie480 on <a href="https://discord.gg/celeste"
...
<p>
    ...
    <a href="https://github.com/EverestAPI/Resources/wiki/Mod-Structure#using-helper-mods" target="_blank">check the
        wiki for more details</a>.
    In the list, you will find sample <code>everest.yaml</code> files that show how to add the latest version of each
    mod as a dependency.
</p>

<p>
    Note that it is <b>not recommended</b> to use <span class="badge bg-danger">Maps</span> as dependencies, because
    anyone wanting to play your map
    would have to get that other map and all its dependencies as well. Maps that ship with custom entities are still
...
<div class="alert alert-info">
    <p>
        This page is mostly generated automatically from map editor plugins, and is refreshed once per day. It was last
        updated on
        <b><span class="timestamp-long"
                 data-timestamp="<%= request.getAttribute("lastUpdatedTimestamp") %>"><%= request.getAttribute("lastUpdated") %></span></b>.
    </p>
    <p>
        If you are a mod maker and want to <b>rename</b> an entity appearing here, make a pull request on
        <a href="https://github.com/maddie480/RandomBackendStuff/blob/main/modcatalogdictionary.txt" target="_blank">that
            file</a>
        in order to add an exception for it.
        If you want to <b>add a button that links to your documentation</b>,
        <a href="https://github.com/EverestAPI/Resources/wiki/Helper-Manuals" rel="noopener" target="_blank">update this
            GitHub wiki page</a>!
    </p>
</div>

<p style="margin-bottom: 0.5rem">
    <b><%= request.getAttribute("entityCount") %>
    </b> entities, <b><%= request.getAttribute("triggerCount") %>
</b> triggers and
    <b><%= request.getAttribute("effectCount") %>
    </b> effects are available through <b><%= request.getAttribute("modCount") %>
</b> mods:
</p>
...
<p>
    <a href="<%= escapeHtml4(mod.pageUrl) %>" rel="noopener" target="_blank" class="btn btn-outline-primary">Mod
        page</a>
    ...
    <a href="<%= escapeHtml4(docLink.getValue()) %>" rel="noopener" target="_blank"
       class="btn btn-outline-secondary"><%= escapeHtml4(docLink.getKey()) %>
    </a>
    <% } %>
</p>
