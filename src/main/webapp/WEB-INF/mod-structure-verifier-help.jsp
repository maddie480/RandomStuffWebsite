...
<div>
    The bot expects your assets to be in a
    <pre><%= escapeHtml4(request.getParameter("collabName")) %>/[subfolder]</pre>
    folder.
    For example:
    <ul>
        <li>for decals:
            <pre>Graphics/Atlases/Gameplay/decals/<%= escapeHtml4(request.getParameter("collabName")) %>/[subfolder]/decal.png</pre>
        </li>
        <li>for color grades:
            <pre>Graphics/ColorGrading/<%= escapeHtml4(request.getParameter("collabName")) %>/[subfolder]/colorgrade.png</pre>
        </li>
        <li>for Lua cutscenes:
            <pre>Assets/<%= escapeHtml4(request.getParameter("collabName")) %>/[subfolder]/mycutscene.lua</pre>
        </li>
        <li>for playback tutorials:
            <pre>Tutorials/<%= escapeHtml4(request.getParameter("collabName")) %>/[subfolder]/mytutorial.bin</pre>
        </li>
    </ul>
</div>
...
<div style="margin-bottom: 20px">
    ...
    <pre>Graphics/Atlases/Gui/emoji/<%= escapeHtml4(request.getParameter("collabName")) %>_[anything]_[anything].png</pre>
    instead.
</div>
...
<div>
    ...
    For example, change the paths to the decals by right-clicking them, edit your playback tutorials, your color grade
    triggers, Lua cutscene triggers, etc.
</div>
<% } %>
...
<div>
    The bot expects your XMLs to be in the
    <pre>Graphics/<%= escapeHtml4(request.getParameter("collabName")) %>xmls/[subfolder]</pre>
    folder,
    for example
    <pre>Graphics/<%= escapeHtml4(request.getParameter("collabName")) %>xmls/[subfolder]/ForegroundTiles.xml</pre>
    .
</div>

<div>To fix this issue, create the missing subfolders and move your XMLs inside it. <b>Don't forget to update your map
    metadata in your map editor to reference the new paths!</b></div>
<% } %>
...
<div>
    Either you forgot to put your map in the zip, or it isn't in the
    <pre>Maps</pre>
    folder.
    <ul>
        <li>Check if your map is actually in the zip!</li>
        <li>
            Check that when opening the zip, you directly see a
            <pre>Maps</pre>
            folder, and that the map bin is in it.
            <i>If you need to open another folder before getting to
                <pre>Maps</pre>
                , that is the issue!</i>
            In that case, build your zip again by selecting everything in your mod folder (including
            <pre>Maps</pre>
...
<div>
    Check that there is a single map bin in
    <pre>Maps</pre>
    .
</div>
<% } %>
...
<div>
    Make sure your map .bin file is placed in
    <pre>Maps/<%=
...
<div>
    The bot expects all your
    <pre>English.txt</pre>
    entries to follow this format:
    <pre><%= escapeHtml4(request.getParameter("collabName")) %>_[prefix]_xxxx</pre>
    .
</div>
<div>
    To fix that, rename the entries that don't follow this format. <b>Don't forget to change anything that refers to
    them</b>, like dialog cutscene triggers,
...
<div>
    ...
    When you open your zip, you should directly see
    <pre>everest.yaml</pre>
    , without needing to open another folder.
</div>
<% } %>
...
<div>
    <pre>everest.yaml</pre>
    is mandatory, as it specifies which helpers are required to play your map.
    You can learn more about dependencies <a target="_blank"
                                             href="https://github.com/EverestAPI/Resources/wiki/Mod-Structure#using-helper-mods">on
    the wiki</a>,
    and check your
    <pre>everest.yaml</pre>
    is valid using <a target="_blank" href="https://maddie480.ovh/celeste/everest-yaml-validator">this online tool</a>.
</div>

<div>
    Lönn allows you to generate and update your
    <pre>everest.yaml</pre>
    file in the <b>Map &gt; Dependencies</b> menu.
    If you use Ahorn, you can <a target="_blank" href="https://gamebanana.com/tools/6908">download the Dependency
    Generator</a> and hit "Generate everest.yaml".
</div>
<% } %>
...
<div>
    The bot sent your
    <pre>everest.yaml</pre>
    to <a target="_blank" href="https://maddie480.ovh/celeste/everest-yaml-validator">the validator</a>
...
<div>
    Your
    <pre>everest.yaml</pre>
    gives multiple IDs to your mod. You should avoid this, since it causes issues with mod loading and updating, and the
    only reason why you
    would want to do this is having multiple DLLs in the same mod zip (which you can usually also avoid &#x1f605;).
</div>

<div>
    This can also happen when the indentation of your
    <pre>everest.yaml</pre>
    file is wrong:
</div>
<pre class="codeblock">
...
<div>
    &#x2b06; This
    <pre>everest.yaml</pre>
    declares that there are 2 mods in this zip:
    <pre>MyMod</pre>
    (depending on
    <pre>Everest</pre>
    ) and
    <pre>MaxHelpingHand</pre>
    .
    In order to have a mod that depends on
    <pre>MaxHelpingHand</pre>
    instead, align it properly to have it inside
...
<div>
    ...
    <ul>
        ...
        <li>You used a custom asset from a helper but it isn't in your
            <pre>everest.yaml</pre>
            : add it there!
        </li>
        <li>You used a decal/styleground but then moved it to another folder or renamed it: fix the paths in your map
            (in the styleground window, or by right-clicking decals).
        </li>
    </ul>
</div>
<% } %>
...
<div>
    ...
    <ul>
        <li>
            You used a custom asset from a helper but it isn't in your
            <pre>everest.yaml</pre>
            : add it there or regenerate your
            <pre>everest.yaml</pre>
            using the
...
<div><i>Note: "Effects" refer to the Effects tab of the Stylegrounds window in Ahorn here. Effects are listed together
    with parallax stylegrounds in Lönn.</i></div>
<% } %>
...
<div>
    Some of the characters you used in your dialogue are not included in the game, so you need to ship them with your
    mod,
...
<div>
    ...
    <pre>--generate-font [language]</pre>
    command with your dialog file attached to have the Mod Structure Verifier generate those files.
    <pre>language</pre>
    should be one of
    <pre>chinese</pre>
    ,
...
<div>
    <i>If you already ship the missing characters with your mod,</i> check that they are at the right place (
    <pre>Mods/yourmod/Dialog/Fonts</pre>
    ),
    with a correct name (one of
    <pre>chinese.fnt</pre>
    ,
    <pre>japanese.fnt</pre>
    ,
    <pre>korean.fnt</pre>
    ,
    <pre>russian.fnt</pre>
    or
    <pre>renogare64.fnt</pre>
    ),
    and check that the .fnt file is valid (it should be in XML format). More details can be found
    <a href="https://github.com/EverestAPI/Resources/wiki/Adding-Custom-Dialogue#custom-font-loading" target="_blank">on
        the wiki</a>.
</div>
<% } %>
