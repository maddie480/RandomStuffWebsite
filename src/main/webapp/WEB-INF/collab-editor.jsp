...
<form method="POST">
    <div class="form-group">
        ...
        <input type="text" class="form-control" id="name" name="name"
               value="<%= escapeHtml4((String) request.getAttribute("name")) %>" required>
    </div>
    <div class="form-group">
        <label for="status"><b>Status</b></label>
        <select class="form-select" id="status" name="status">
            <option value="in-progress"
                    <% if (request.getAttribute("status").equals("in-progress")) { %>selected<% } %>>In Progress
            </option>
            <option value="paused" <% if (request.getAttribute("status").equals("paused")) { %>selected<% } %>>Paused
            </option>
            <option value="released" <% if (request.getAttribute("status").equals("released")) { %>selected<% } %>>
                Released
            </option>
            <option value="cancelled" <% if (request.getAttribute("status").equals("cancelled")) { %>selected<% } %>>
                Cancelled
            </option>
            <option value="hidden" <% if (request.getAttribute("status").equals("hidden")) { %>selected<% } %>>Hidden
            </option>
        </select>
    </div>
    <div class="form-group">
        ...
        <input type="text" class="form-control" id="contact" name="contact"
               value="<%= escapeHtml4((String) request.getAttribute("contact")) %>" required>
    </div>

    <div class="form-group">
        <div class="form-check">
            <input class="form-check-input" type="checkbox" id="lookingForPeople" name="lookingForPeople"
                   <% if (request.getAttribute("lookingForPeople").equals("yes")) { %>checked<% } %>>
    ...
    <div class="form-group">
        <label for="req<%= req %>" class="open-to"><b>Open to <%= req %>
        </b></label>
